...
public abstract class BaseComparisonExecutor<TInput, TType extends Comparable<TType>> implements IScalarExecutor<TInput, Boolean> {
	
	private IScalarExecutor<TInput, TType> left;
	private IScalarExecutor<TInput, TType> right;

	public BaseComparisonExecutor(IScalarExecutor<TInput, TType> left, IScalarExecutor<TInput, TType> right) {
		this.setLeft(left);
		this.setRight(right);
	}
