...
public abstract class BaseBinaryConditionExecutor<TInput> implements IScalarExecutor<TInput, Boolean> {
	
	private IScalarExecutor<TInput, Boolean> left;
	private IScalarExecutor<TInput, Boolean> right;
	
	public BaseBinaryConditionExecutor(IScalarExecutor<TInput, Boolean> left, IScalarExecutor<TInput, Boolean> right) {
		this.setLeft(left);
		this.setRight(right);
	}
