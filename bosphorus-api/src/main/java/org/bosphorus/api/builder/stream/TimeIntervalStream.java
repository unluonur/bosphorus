...
public class TimeIntervalStream<TInput, TOutput> extends BaseSingleOutputStream<TInput, TOutput> {
	...
	public TimeIntervalStream(IAggregateExpression<TInput, TOutput> expression, int interval) {
	...
	public IPipeExecutor<TInput> build(INodeContext context) throws Exception {
		return new TimeIntervalBatch<TInput, TOutput>(this.expression.build().create(), this.getOutput().build(context), interval);
	}
